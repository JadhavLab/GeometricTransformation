...
%  trajectory distance based on UMAP manifolds of neural        %
...
for track = [1,3]
    ...
    for pair = 1:length(pairind(:,1))
        ...
        for i = 1:length(traj_label(:,1))
            ...
            if ~isempty(label_ids) && ~isempty(label_ids_alt) % if the bin on both trials is not empty
                spike_currenttraj= spike_umap(label_ids,:);
                spike_alttraj= spike_umap(label_ids_alt,:);
                dist_tmp = [dist_tmp;sum( (spike_currenttraj'-spike_alttraj').^2).^0.5]; % Euclidean distance
            end
        end
        umap_dist_inseq = [umap_dist_inseq;nanmean(dist_tmp(:))];
    end
end

%%
% calculate OUTSeq distance
umap_dist_outseq = []; % reset matrix
for track = [1,3]
    if track == 1
    ...
    for pair = 1:length(pairind(:,1))
        ...
        umap_dist_outseq = [umap_dist_outseq;nanmean(dist_tmp(:))];
    end
end
%%
%-------plot results-------%
figure('Position',[300,300,200,300]),
groupnames = [ones(size(umap_dist_inseq));2 * ones(size(umap_dist_outseq))];
boxplot([umap_dist_inseq;umap_dist_outseq],groupnames,'Labels',{'InSeq','OutSeq'})
ylim([0,25])
ylabel('Distance (a.u.)')
if CTXHP
   title('PFC')
...
up_adjYData{1}(1,2)= quantile(umap_dist_outseq,0.9);% Setting the upper whiskers
up_adjYData{2}(1,2)= quantile(umap_dist_inseq,0.9);% Setting the upper whiskers
[up_adj.YData] = deal(up_adjYData{:});  

up_adj = findobj(gca,'type', 'line', 'tag', 'Upper Adjacent Value'); 
up_adjYData = {up_adj.YData};
up_adjYData{1}(:)= quantile(umap_dist_outseq,0.9);% Setting the upper whiskers
up_adjYData{2}(:)= quantile(umap_dist_inseq,0.9);% Setting the upper whiskers
[up_adj.YData] = deal(up_adjYData{:});  

low_adj = findobj(gca,'type', 'line', 'tag', 'Lower Whisker'); 
low_adjYData = {low_adj.YData};
low_adjYData{1}(1,1)= quantile(umap_dist_outseq,0.1);% Setting the lower whiskers
low_adjYData{2}(1,1)= quantile(umap_dist_inseq,0.1);% Setting the lower whiskers
[low_adj.YData] = deal(low_adjYData{:});  

low_adj = findobj(gca,'type', 'line', 'tag', 'Lower Adjacent Value'); 
low_adjYData = {low_adj.YData};
low_adjYData{1}(:)= quantile(umap_dist_outseq,0.1);% Setting the lower whiskers
low_adjYData{2}(:)= quantile(umap_dist_inseq,0.1);% Setting the lower whiskers
[low_adj.YData] = deal(low_adjYData{:});      
