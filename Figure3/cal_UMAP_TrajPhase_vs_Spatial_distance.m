...
%  trajectory phase on UMAP neural manifolds. The UMAP MATLAB   %
%  toolbox (V4.1) is needed (https://www.mathworks.com/         %
%  matlabcentral/fileexchange/71902-uniform-manifold-           %
...
for i = 1:length(traj_label(:,1))
    for track = [1,3]
        ...
        spike_umap_currenttraj= spike_umap(label_ids,:);
        if track == 1
        ...
        spike_umap_alttraj= spike_umap(label_ids_alt,:);
        dist_tmp = pdist2(spike_umap_alttraj,spike_umap_currenttraj,'euclidean'); % Euclidean distance
        umap_dist_spatial = [umap_dist_spatial;dist_tmp(:)];
    end
end
%%
...
for i = 1:length(traj_label(:,1))
    for track = [1,3]
        ...
        spike_umap_alttraj= spike_umap(label_ids_alt,:);
        dist_tmp = pdist2(spike_umap_alttraj,spike_umap_currenttraj,'euclidean'); % Euclidean distance
        umap_dist_phase = [umap_dist_phase;dist_tmp(:)];
    end
end
%%
%-------plot results-------%
figure('Position',[300,300,200,300]),
groupnames = [ones(size(umap_dist_spatial));2 * ones(size(umap_dist_phase))];
boxplot([umap_dist_spatial;umap_dist_phase],groupnames,'Labels',{'Spatial','Traj-phase'})
ylim([0,30])
ylabel('Distance (a.u.)')
if CTXHP
   title('PFC')
...
up_adjYData{1}(1,2)= quantile(umap_dist_phase,0.9);% Setting the upper whiskers
up_adjYData{2}(1,2)= quantile(umap_dist_spatial,0.9);% Setting the upper whiskers
[up_adj.YData] = deal(up_adjYData{:});  

up_adj = findobj(gca,'type', 'line', 'tag', 'Upper Adjacent Value'); 
up_adjYData = {up_adj.YData};
up_adjYData{1}(:)= quantile(umap_dist_phase,0.9);% Setting the upper whiskers
up_adjYData{2}(:)= quantile(umap_dist_spatial,0.9);% Setting the upper whiskers
[up_adj.YData] = deal(up_adjYData{:});  

low_adj = findobj(gca,'type', 'line', 'tag', 'Lower Whisker'); 
low_adjYData = {low_adj.YData};
low_adjYData{1}(1,1)= quantile(umap_dist_phase,0.1);% Setting the lower whiskers
low_adjYData{2}(1,1)= quantile(umap_dist_spatial,0.1);% Setting the lower whiskers
[low_adj.YData] = deal(low_adjYData{:});  

low_adj = findobj(gca,'type', 'line', 'tag', 'Lower Adjacent Value'); 
low_adjYData = {low_adj.YData};
low_adjYData{1}(:)= quantile(umap_dist_phase,0.1);% Setting the lower whiskers
low_adjYData{2}(:)= quantile(umap_dist_spatial,0.1);% Setting the lower whiskers
[low_adj.YData] = deal(low_adjYData{:});  
