function decodinginfo = decoding_position_PFC_novelfamiliar_cross(animalprefix,day,ep,linfields,decoded_trajtime)
%---------------------------------------------------------------%
%  This is the function for PFC decoding of spatial locations   %
...
for ttt = 1:length(tetinfo{day}{ep_curr})
     if ~isempty(tetinfo{day}{ep_curr}{ttt})
        if isfield(tetinfo{day}{ep_curr}{ttt},'area')
            if strcmp(tetinfo{day}{ep_curr}{ttt}.area,'PFC')
...
for tet = detect_tet  % tetrode iterations
    ...
    if ~isempty(spikes)
        if  strcmp(tetinfo{day}{ep_curr}{tet}.area,'PFC')
...
if ~isempty(exclude_list)
   ctxidx = setdiff(cellind,exclude_list,'rows');
else
   ctxidx = cellind;
end
